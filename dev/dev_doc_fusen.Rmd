...
xlsform_compare <- function( listfile, 
    ...
    #### Variables master to... ######
    variablesmaster <-  readxl::read_excel(xlsformpath, sheet = "survey") |>
      ## Rename and use what ever label set is coming first
      dplyr::rename(label = ifelse( is.null(label_language), dplyr::first(tidyselect::starts_with("label")), paste0("label",label_language)),
                    hint =  ifelse( is.null(label_language), dplyr::first(tidyselect::starts_with("hint")), paste0("hint",label_language)) ,
                    constraint_message =  dplyr::first(tidyselect::starts_with("constraint_message"))
    ...
    for ( i in 2:length(listfile ))  {
     ...
     # i <- 2
      ...
      #                  list_name,
      #                  label, label.diff,
      #                  form_file, form_file.diff)|>
        dplyr::filter(!(is.na(list_name))) |>
        ##  clean the diff type comparison
        dplyr::mutate (label.diff = dplyr::if_else(is.na(label),label.diff , dplyr::if_else( label.diff == label, NA_character_, label.diff )))
      ...
      ## Now variable ####
      variables <-  readxl::read_excel(xlsformpath, sheet = "survey") |>
        ## Rename and use what ever label set is coming first
        dplyr::rename(label = ifelse( is.null(label_language), dplyr::first(tidyselect::starts_with("label")), paste0("label",label_language)),
                      hint =  ifelse( is.null(label_language), dplyr::first(tidyselect::starts_with("hint")), paste0("hint",label_language)) ,
                     # constraint_message =  dplyr::first(tidyselect::starts_with("constraint_message"))
      ...
      variablescompare <- dplyr::full_join(variablescompare, variables, by = c( "name"), suffix=c("",".diff") ) |>
        # dplyr::select( name,
        #                type, type.diff,
        #                list_name, list_name.diff,
        #                label, label.diff,
        #                hint, hint.diff,
        #                required, required.diff,
        #                relevant, relevant.diff,
        #                constraint, constraint.diff,
        #                #  instruct, instruct.diff,
        #                form_file, form_file.diff) |>
        dplyr::filter(!(is.na(name))) |>
        ##  clean the diff type comparison
        #dplyr::mutate (label.diff2 = ifelse( label.diff == label, NA_character_, label.diff ))
...
check <- xlsform_compare(listfile = c( "demo.xlsx","demo_adapt1.xlsx","demo_adapt2.xlsx"),  
                         folder = stringr::str_sub(system.file("", package = "XlsFormPrettyPrint"), end = -2) ,                    
                         label_language = NULL,
                         fileout = NULL)
```
